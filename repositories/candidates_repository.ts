...
type CandidateRow = { id: string; candidate_status: CandidateStatus; first_seen_at: string | null; updated_at: string; reviewed_at: string | null; saved_at: string | null; expired_at: string | null; last_fetched_at: string | null; title: string; body: string; summary: string | null; url: string; source: string | null; published_at: string | null; tags_json: string; candidate_score: number | null; candidate_match_reason: string | null; saved_card_id: string | null };
function toCandidate(row: CandidateRow): CandidateLifecycle {
  let tags: string[] = [];
  try { tags = JSON.parse(row.tags_json || '[]'); } catch { tags = []; }
  return { score: row.candidate_score ?? undefined, matchReason: row.candidate_match_reason ?? undefined, savedCardId: row.saved_card_id ?? undefined, candidateId: row.id, status: row.candidate_status, firstSeenAt: row.first_seen_at ?? row.updated_at, lastSeenAt: row.last_fetched_at ?? row.updated_at, reviewedAt: row.reviewed_at ?? undefined, savedAt: row.saved_at ?? undefined, expiredAt: row.expired_at ?? undefined, sourceArticleId: row.id, title: row.title, body: row.body, summary: row.summary ?? undefined, url: row.url, source: row.source ?? undefined, publishedAt: row.published_at ?? undefined, tags };
}
const select = `SELECT id, candidate_status, first_seen_at, updated_at, reviewed_at, saved_at, expired_at, last_fetched_at, title, body, summary, url, source, published_at, tags_json, candidate_score, candidate_match_reason, saved_card_id FROM articles`;
...
export function saveCandidate(id: string, savedCardId: string): CandidateLifecycle | null {
  const now = new Date().toISOString();
  const result = db.prepare(`UPDATE articles SET candidate_status = 'saved_as_card', saved_at = ?, saved_card_id = ?, updated_at = ? WHERE id = ? AND candidate_status IN ('unreviewed', 'reviewed_not_saved') AND saved_card_id IS NULL`).run(now, savedCardId, now, id);
  return result.changes ? getCandidate(id) : null;
}
