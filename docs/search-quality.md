...
`npm run build:search-fixture` reads the local SQLite cards table and writes the versioned anonymized corpus at `data/search-evaluation/anonymized-corpus.json`. The fixture contains no raw IDs, title/body text, or URLs; it preserves token arrays, document lengths, relative update age, and archive status so the real scope is reproducible in CI. The current fixture contains 233 documents and 18,138 tokens. Relevance scores are not automatically substituted for the hand-labeled fixture: doing so from a card's own title would be circular, so manually labeled query cases are still required for a trustworthy real-card score.
