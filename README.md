...
Benchmark evidence is separated into ranking-only, production-like candidate pipeline, end-to-end API, cold-start, and warm-search scopes:

- End-to-end local app measurement: measures the actual application path and includes additional application overhead.
- Ranking-only: deterministic synthetic corpora with precomputed token data.
- Production-like: deduplication, score filtering, and result limiting enabled.
- End-to-end API: load and pipeline timing.
- Cold-start and warm-search: first-run and steady-state comparisons.

| Measurement | Scope | Result |
|---|---|---:|
| End-to-end local app path | Actual app path | 1.413 s |
| Ranking-only | 10,000 synthetic cards | generated by `npm run benchmark` |
...
For the reproducible portfolio walkthrough, see [docs/demo-15min.md](docs/demo-15min.md).
